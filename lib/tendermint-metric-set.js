...
let candidatesExtractor = (candidates, m, fieldToExtract) => {
	...
	candidates.forEach(v => {
		if (v[`${fieldToExtract}`] !== undefined)
			m.set({ reward_address : v.reward_address, owner_address: v.owner_address, pub_key: v.pub_key, commission: v.commission, created_at_block: v.created_at_block }, v[`${fieldToExtract}`], x => parseInt(x))
	})
	return false
}
