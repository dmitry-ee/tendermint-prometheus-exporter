...
let candidatesExtractor = (candidates, m) => {
	if (!isArray(candidates))
		return false

	candidates.forEach(v => {
		m.set({ reward_address : v.reward_address, owner_address: v.owner_address, total_stake: v.total_stake, pub_key: v.pub_key, commission: v.commission, created_at_block: v.created_at_block }, v.status, x => x)
	})
	return false
}
...
const metrics = {
	'candidates': [
		{	name: 'info', help: 'Candidate Info',	extractPath: 'result', labelNames: ['reward_address', 'owner_address', 'total_stake', 'pub_key', 'commission', 'created_at_block'],	extractor: (v, m) => candidatesExtractor(v, m) }
	],
	'net_info': [
