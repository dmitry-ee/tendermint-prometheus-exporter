...
class Metric {
  ...
  setLabels(newLabels = {}) {
    ...
    if (this._initPrometheusMetric) {
      logger.debug(`trying to modify labels with ${JSON.stringify(newLabels)}, _labels=${JSON.stringify(this._labels)} on already initialized metric [metricName=${this._name}]`)
  ...
  set(additionalLabels = {}, extractObject, currentExtractor) {
    ...
    if (isUndefined(extractObject)) {
      ...
      if (isBoolean(metricValue) && !metricValue && this._metricDataType !== Boolean) {
        logger.debug(`value '${metricValue}' for [metricName=${this._name}] == false! Ignoring...`)
        return false
      }
      if (isNaN(metricValue)) {
        logger.debug(`value '${metricValue}' for [metricName=${this._name}] == NaN! Ignoring...`)
        return false
      }
